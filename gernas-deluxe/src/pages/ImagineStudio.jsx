...
function generateAgentSpec(step) {
  ...
  const systemPrompt = `You are ${name}, an intelligent automation agent in the Deluxe Corporation DLX AGENTIC OS platform.
...
<div class="wrap">
  ...
  <!-- ── Cover ── -->
  <div class="cover">
    <div class="cover-eyebrow">DLX AGENTIC OS · Deluxe Corporation · Confidential</div>
  ...
  <!-- ── Footer ── -->
  <div class="footer">
    <span>DLX AGENTIC OS · Deluxe Corporation · Confidential &amp; Internal Use Only</span>
  ...
  const handleSendToConfluence = () => {
    ...
    const systemPrompt = `You are an AI agent designed to automate the following business requirement identified via the DLX AGENTIC OS Imagination Studio.
...
SOURCE: Generated from Nova Discovery session in Imagination Studio`
  ...
  const handleOpenInVelox = (spec) => {
    ...
    const collaborationAuthor = step.status === 'partial' && originalAgent
      ? (originalAgent.author || 'DLX AGENTIC OS Team')
      : undefined
    setSpecModal(null)
    navigate('/builder', {
      state: {
                ...
                <div className="flex flex-col gap-2">
                  {EXAMPLE_PROMPTS.map((p, i) => (
                    <button
                      ...
                      disabled={typing}
                      className="text-left px-3 py-2.5 rounded-xl border border-[#E2E8F0] hover:border-[#C8102E] hover:bg-[#FDF0F2] text-xs text-[#4A5568] transition-all leading-relaxed disabled:opacity-40"
                    >
                      "{p}"
                    </button>
                  ))}
