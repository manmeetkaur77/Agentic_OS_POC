...
const PHASES = [
  { id: 1, label: 'Agent Designer',        desc: 'Identity, skills & behavior' },
...
const MODELS = [
  { id: 'claude-sonnet-4-6', label: 'Claude Sonnet 4.6', tier: 'Recommended', badge: 'bg-emerald-100 text-emerald-700', desc: 'Best balance of speed and intelligence for most agent tasks.' },
...
2. Compute a churn risk score (0–100) for each merchant based on behavioral signals.
...
- Flag any anomalies you cannot categorize for human review.
- Prioritize accuracy over speed — a missed churn signal is better than a false positive.`,
...
function PhaseDesigner({ form, set, navigate }) {
          ...
          {SIDEBAR_SECTIONS.map(s => {
            ...
            return (
              <button
                ...
                style={active ? { background: '#1A2340' } : {}}
              >
                <Icon size={14} className={active ? 'text-white' : 'text-[#9BA8BA]'} />
                {s.label}
              </button>
            )
          })}
          ...
          >
            ...
            {section === 'identity' && (
              <div className="space-y-5">
                <div>
                  <h2 className="font-semibold text-[#1A2340] mb-0.5">Agent Identity</h2>
                  <p className="text-xs text-[#718096]">Define who this agent is and what it does</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-[#4A5568] mb-1.5">Agent Name <span className="text-[#C8102E]">*</span></label>
                    <input value={form.name} onChange={e => set('name', e.target.value)}
                      placeholder="e.g. Invoice Reconciliation Agent"
                      className="w-full px-3 py-2.5 rounded-xl border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#C8102E] transition-colors" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#4A5568] mb-1.5">Version</label>
                    <input value={form.version} onChange={e => set('version', e.target.value)}
                      placeholder="1.0.0"
                      className="w-full px-3 py-2.5 rounded-xl border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#C8102E] transition-colors" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#4A5568] mb-1.5">Author</label>
                    <input value={form.author} onChange={e => set('author', e.target.value)}
                      placeholder="DLX_AGENTIC_OS Team"
                      className="w-full px-3 py-2.5 rounded-xl border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#C8102E] transition-colors" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#4A5568] mb-1.5">Value Stream <span className="text-[#C8102E]">*</span></label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#4A5568] mb-1.5">Description <span className="text-[#C8102E]">*</span></label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#4A5568] mb-1.5">System Prompt <span className="text-[#C8102E]">*</span></label>
                  <textarea value={form.systemPrompt} onChange={e => set('systemPrompt', e.target.value)}
                    placeholder={`You are a ${form.name || 'Deluxe AI Agent'} for Deluxe Corporation.\n\nYour role is to...\n\nPrioritize accuracy. Log all actions. Flag exceptions for human review.`}
                    rows={6}
            ...
            {section === 'skills' && (
              <div className="space-y-4">
                <div>
                  <h2 className="font-semibold text-[#1A2340] mb-0.5">Skills & Tools</h2>
                  <p className="text-xs text-[#718096]">Select the capabilities this agent can use</p>
                </div>
                      ...
                      {(form.skills || []).map(id => {
                        ...
                        return (
                          <span key={id} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium text-white" style={{ background: '#1A2340' }}>
                            {sk?.label}
                            <button onClick={() => toggleSkill(id)} className="ml-0.5 hover:text-red-300"><X size={10} /></button>
                          </span>
                        )
                      })}
                    </div>
                ...
                <div className="pt-2 border-t border-[#E2E8F0]">
                  ...
                  {TRIGGER_TYPES.find(t => t.id === triggerType)?.opts.length > 0 ? (
                    ...
                    <div className="flex gap-2">
                      <input placeholder={triggerType === 'webhook' ? 'https://your-endpoint.com/hook' : 'Label or stream name'}
                      ...
                      <button onClick={e => { const inp = e.target.previousSibling; addTrigger(inp.value); inp.value = '' }}
                        className="px-3 py-2 rounded-lg text-white text-xs font-medium" style={{ background: '#C8102E' }}>Add</button>
                    </div>
                  )}
                </div>
              </div>
            ...
            {section === 'intelligence' && (
              <div className="space-y-4">
                <div>
                  <h2 className="font-semibold text-[#1A2340] mb-0.5">Intelligence Layer</h2>
                  <p className="text-xs text-[#718096]">Choose the AI model that powers this agent</p>
                </div>
                {MODELS.map(m => (
                  <button key={m.id} onClick={() => set('model', m.id)}
                    className={`w-full p-5 rounded-xl border-2 text-left transition-all ${form.model === m.id ? 'border-[#1A2340] bg-[#F7F9FF]' : 'border-[#E2E8F0] hover:border-[#CBD5E0]'}`}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <Bot size={15} className={form.model === m.id ? 'text-[#1A2340]' : 'text-[#718096]'} />
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {form.model === m.id && <CheckCircle size={15} className="text-[#1A2340]" />}
                      </div>
                    </div>
            ...
            {section === 'guardrails' && (
              <div className="space-y-4">
                <div>
                  <h2 className="font-semibold text-[#1A2340] mb-0.5">Guardrails</h2>
                  <p className="text-xs text-[#718096]">Safety controls and operational boundaries</p>
                </div>
                <div className="space-y-2">
                  {[
                    { key: 'humanReview',      label: 'Require human review for high-risk actions',       desc: 'Pauses agent and sends approval request before irreversible actions' },
                    ...
                    { key: 'dryRunMode',       label: 'Start in dry-run mode (read only)',               desc: 'Agent reads and reasons but makes no writes until approved' },
                    { key: 'alertOnException', label: 'Alert admin on unhandled exceptions',             desc: 'Pings platform admin when agent encounters unexpected state' },
                  ].map(({ key, label, desc }) => (
...
function PhaseSimulation({ form }) {
  ...
  const send = async () => {
    ...
    // Build a guardrail-aware system prompt prefix
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#718096] mt-0.5">Send test inputs and observe agent behavior before deploying</p>
        </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg border text-xs transition-all ${
                      simGuardrails[key] ?? on
                        ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
                        : 'border-[#E2E8F0] bg-[#F7F8FA] text-[#9BA8BA]'
                    }`}
                  >
        ...
        <div className="card p-5 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <select value={form.dataSource || 'synthetic'}
                onChange={e => set('dataSource', e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#C8102E] bg-white">
                <option value="synthetic">Synthetic test data</option>
                <option value="anonymized">Anonymized production copy</option>
            ...
            <div>
              ...
              <select value={form.validationPeriod || '48h'}
                onChange={e => set('validationPeriod', e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#C8102E] bg-white">
...
function PhaseProd({ form, onDeploy }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="font-semibold text-[#1A2340]">Production Deployment</h2>
        <p className="text-xs text-[#718096] mt-0.5">Review your agent configuration and deploy to production</p>
      </div>
      ...
      <div className="card p-5 bg-emerald-50 border-emerald-200">
        <div className="flex items-center justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-emerald-800">Ready to deploy to production</p>
              <p className="text-xs text-emerald-700 mt-0.5">This agent will go live immediately and start processing real data.</p>
            </div>
          </div>
...
export default function AgentBuilder() {
  ...
  const defaultForm = template ? {
    ...
    segment: template.segment, model: template.model,
    skills: [...(template.tools || [])], triggers: [...(template.triggers || [])],
    systemPrompt: '', maxConcurrent: 3, version: '1.0.0', author: 'DLX_AGENTIC_OS Team',
  } : prefill ? {
    name: prefill.name || '', description: prefill.description || '',
    segment: prefill.segment || 'merchant', model: 'claude-sonnet-4-6',
    skills: [], triggers: [], systemPrompt: '', maxConcurrent: 1,
    version: '1.0.0', author: 'DLX_AGENTIC_OS Team',
  } : { ...SAMPLE_AGENT }
  ...
  if (deployed) {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <p className="text-sm text-[#CBD5E0]">Redirecting to Agent Pool...</p>
      </motion.div>
    )
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="font-display text-xl font-bold text-[#1A2340]">Agent Builder</h1>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={phase}
          ...
          {phase === 1 && <PhaseDesigner form={form} set={set} navigate={navigate} />}
          {phase === 2 && <PhaseSimulation form={form} />}
          {phase === 3 && <PhaseNonProd form={form} set={set} />}
          {phase === 4 && <PhaseProd form={form} onDeploy={handleDeploy} />}
        </motion.div>
      </AnimatePresence>
